...
// ENTRY
export type ITypedMapEntry<GKey, GValue> = readonly [
  key: GKey,
  value: GValue,
];
...
// METHODS
export type InferTypedMapHasValueFromKey<GEntry extends IGenericTypedMapEntry, GKey> =
  GEntry extends readonly [GKey, any]
    ? true
    : false;

export type InferTypedMapValueFromKey<GEntry extends IGenericTypedMapEntry, GKey extends InferTypedMapKeys<GEntry>> =
  GEntry extends readonly [GKey, infer GValue]
    ? GValue
    : never;
...
export class TypedMap<GEntry extends IGenericTypedMapEntry> {
  ...
  constructor(
    input: Iterable<GEntry>,
  ) {
    this.#map = (input instanceof Map)
      ? input
      : new Map<unknown, unknown>(input);
  }
  ...
  has<GKey>(
    key: GKey,
  ): InferTypedMapHasValueFromKey<GEntry, GKey> {
    return this.#map.has(key) as InferTypedMapHasValueFromKey<GEntry, GKey>;
  }

  get<GKey extends InferTypedMapKeys<GEntry>>(
    key: GKey,
  ): InferTypedMapValueFromKey<GEntry, GKey> {
  ...
  delete<GKey extends InferTypedMapKeys<GEntry>>(
    key: GKey,
  ): void {
    this.#map.delete(key);
  }
...
export class ReadonlyTypedMap<GEntry extends IGenericTypedMapEntry> extends TypedMap<GEntry> {
  ...
  override delete<GKey extends InferTypedMapKeys<GEntry>>(
    key: GKey,
  ): void {
    throw new Error(`Readonly`);
  }
